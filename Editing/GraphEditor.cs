...
namespace Editing {
	/// <summary>
	///     This class implements a graph editor control. It is a work in progress, especially with regards to allowing users
	///     to configure the editor's behaviour.
	...
	internal class GraphEditor : Panel {
		...
		private readonly Image _treeImage;

		private readonly float radiusRatio = 0.3f;
		...
		public GraphEditor() : this(null, null, null) {
			...
			_treeImage = Image.FromFile("tree.jpg");
		}
		...
		internal GraphEditor(
			ShowEditor showEditorDelegate,
			ValidateEditor validateEditorDelegate,
			...
			(Viewer as IViewer).MouseDown += Form1_MouseDown;
			(Viewer as IViewer).MouseUp += Form1_MouseUp;
		...
		/// </summary>
		internal GViewer Viewer { get; } = new GViewer();
		...
		internal Graph Graph {
			get => Viewer.Graph;
			set => Viewer.Graph = value;
		}

		/// <summary>
		///     The ToolBar contained in the viewer.
		/// </summary>
		internal ToolStrip ToolBar => Viewer.Controls.OfType<ToolStrip>().FirstOrDefault();
		...
		private void ToolBar_ButtonClick(object sender, ToolStripItemClickedEventArgs e) {
			foreach (NodeTypeEntry nte in m_NodeTypes)
				if (nte.Button == e.ClickedItem) {
					...
					var rect1 = Viewer.ClientRectangle;//gViewer.Graph.GeometryGraph.BoundingBox;
					var rect2 = Viewer.Graph.BoundingBox;
					var p = Viewer.ScreenToSource(rect1.Location);
					var p2 = Viewer.ScreenToSource(rect1.Location + rect1.Size);
					if (p.X < rect2.Left)
						p.X = rect2.Left;
					if (p2.X > rect2.Right)
						p2.X = rect2.Right;
		...
		private string GetNewId() {
			string ret = "_ID" + _idCounter++;
			if (Viewer.Graph.FindNode(ret) != null)
				return GetNewId();
			return ret;
		}
		...
		internal virtual DrawingNode InsertNode(Point center, NodeTypeEntry nte, string id) {
			...
			node.UserData = s;
			var dNode = Viewer.CreateIViewerNode(node, center, null);
		...
		/// <param name="nte">The NodeTypeEntry structure containing the initial aspect of the node</param>
		/// <returns>The new node</returns>
		internal virtual DrawingNode InsertNode(Point center, NodeTypeEntry nte) => InsertNode(center, nte, GetNewId());
		...
		private void insertNode_Click(object sender, EventArgs e) {
			...
			foreach (NodeTypeEntry nte in m_NodeTypes)
				if (nte.MenuItem == sender)
					selectedNTE = nte;
			var n = InsertNode(m_MouseRightButtonDownPoint, selectedNTE);
			if (NodeInsertedByUser != null)
				NodeInsertedByUser(n);
		}
		...
		private void deleteSelected_Click(object sender, EventArgs e) {
			...
			foreach (var ob in Viewer.Entities)
		...
		private void m_LabelBox_KeyDown(object sender, KeyEventArgs e) {
			if (e.KeyValue == 13)
				if (ValidateEditorDelegate())
					CloseEditorDelegate();
		}

		private void Form1_MouseUp(object sender, MsaglMouseEventArgs e) {
			object obj = Viewer.GetObjectAt(e.X, e.Y);
		...
		/// <summary>
		///     Overloaded. Adds a new node type to the list. If the parameter contains an image, a button with that image will be
		///     added to the toolbar.
		/// </summary>
		/// <param name="nte">
		///     The NodeTypeEntry structure containing the initial aspect of the node, type name, and additional parameters
		///     required
		///     for node insertion.
		/// </param>
		...
		/// <summary>
		///     Overloaded. Adds a new node type to the list. If the parameters contain an image, a button with that image will be
		///     added to the toolbar.
		/// </summary>
		...
		private void Form1_MouseDown(object sender, MsaglMouseEventArgs e) {
			...
			if (e.RightButtonIsPressed && !e.Handled) {
				m_MouseRightButtonDownPoint = Viewer.ScreenToSource(e);

				var cm = BuildContextMenu(m_MouseRightButtonDownPoint);
		...
		public ICurve GetNodeBoundary(DrawingNode node) {
			double width = _treeImage.Width / 2.0;
			double height = _treeImage.Height / 2.0;
			...
			using (var m = g.Transform) {
				using (var saveM = m.Clone()) {
					...
					using (var m2 = new Matrix(1, 0, 0, -1, (float)leftTop.X, (float)leftTop.Y)) {
						...
						using (var lBrush = new LinearGradientBrush(
								rect,
		...
		/// <summary>
		///     Contains all the information needed to allow the user to insert a node of a specific type. This includes a name
